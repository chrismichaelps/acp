...
export const WorkUnit: Schema.Struct<{
  ...
  description: optionalWith<string, Option>
  state: WorkState
  priority: Priority
  created_by: WorkerId
  assigned_to: optionalWith<WorkerId, Option>
...
including `changes_requested` from spec §14; `assigned_to` is `Option`. Payload
schemas are the decode targets for `POST /v1/work` and `/claim` (spec §12.3–12.4).
...
[[event.schema]] · [[work-unit-service]] · [[Storage]] · [[src/_MOC]]
