...
`checkpoint.create` authorizes `checkpoint:create`, mints an id and timestamp
through [[id-clock]], and delegates to [[checkpoint-service]] so append-only
persistence and `checkpoint.created` event emission remain domain-owned.

Read handlers authorize `workspace:read`. Work-scoped reads first prove the
WorkUnit exists through [[work-unit-service]], matching the HTTP resume route.
`checkpoint.list_for_work` and `checkpoint.list_for_workspace` preserve
newest-first ordering from [[checkpoint-service]]. `checkpoint.latest_for_work`
