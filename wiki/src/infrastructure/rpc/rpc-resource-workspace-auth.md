...
The helper loads the target work unit, lease, artifact, or review, derives its
...
`review(headers, scope, reviewId)` returns the authorized actor plus the loaded
[[review-service]] record and parent [[work-unit-service]] record. All helpers
map absent resources through [[rpc-error]] to canonical `not_found` responses,
while workspace mismatches remain `forbidden`.
...
release, revocation, artifact mutation, evidence read, or review outcome.

## Negative Logic (Prohibited Paths)

- ❌ Do NOT duplicate resource loading in each work, lease, artifact, or review
  RPC handler.
