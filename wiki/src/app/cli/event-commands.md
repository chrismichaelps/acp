...
`events list --workspace <id> [--after <seq>]` maps to
`GET /v1/events?workspace_id=&after_seq=`. `events stream --workspace <id>`
...
`events list` defaults `--after` to `0` and validates any provided cursor as a
non-negative safe integer. `events stream` marks the request as streaming so
[[cli-main]] can keep the response open instead of routing it through the normal
one-shot client.
