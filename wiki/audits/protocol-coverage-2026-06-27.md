...
artifact create, checkpoint create, review request, and event subscription over
...
transport only exposes `review.request`. That is acceptable for the current MVP
if review action is host-internal, but it should be made explicit before calling
review transport coverage complete.
